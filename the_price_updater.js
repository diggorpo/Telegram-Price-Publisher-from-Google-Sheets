...
class TelegramService {
  constructor(token, chatId) {
    this.token = token;
    this.chatId = chatId;
    this.tgBaseUrl = `https://api.telegram.org/bot${this.token}/`;
  }
  ...
  apiRequest(method, payload) {
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
      ...
      try {
        const response = UrlFetchApp.fetch(this.tgBaseUrl + method, options);
  ...
  sendMessage(text, reply_markup = null) {
    console.log(`Отправка нового сообщения в чат ${this.chatId}...`);
    const payload = { chat_id: this.chatId, text: text, parse_mode: 'HTML' };
    if (reply_markup) payload.reply_markup = reply_markup;
    return this.apiRequest('sendMessage', payload);
  }

  editMessage(messageId, text) {
    console.log(`Редактирование сообщения ${messageId} в чате ${this.chatId}...`);
    return this.apiRequest('editMessageText', { chat_id: this.chatId, message_id: messageId, text: text, parse_mode: 'HTML' });
  }

  deleteMessage(messageId) {
    console.log(`Удаление сообщения ${messageId} из чата ${this.chatId}...`);
    return this.apiRequest('deleteMessage', { chat_id: this.chatId, message_id: messageId });
  }

  pinMessage(messageId) {
    console.log(`Закрепление сообщения ${messageId} в чате ${this.chatId}...`);
    return this.apiRequest('pinChatMessage', { chat_id: this.chatId, message_id: messageId, disable_notification: true });
  }

  unpinMessage(messageId) {
    console.log(`Открепление сообщения ${messageId} из чата ${this.chatId}...`);
    return this.apiRequest('unpinChatMessage', { chat_id: this.chatId, message_id: messageId });
  }
}

class Product {
  constructor(row, colMap) {
    // Твой код здесь.

    // Пример для имени:
    this.name = row[colMap[CONFIG.COLUMN_NAMES.NAME]];
    this.flag = row[colMap[CONFIG.COLUMN_NAMES.FLAG]] || "";;
    this.category = row[colMap[CONFIG.COLUMN_NAMES.CATEGORY]];

    const rawPrice = parseInt(row[colMap[CONFIG.COLUMN_NAMES.PRICE]])
    this.price = isNaN(rawPrice) ? null : rawPrice;
  }
  ...
  getFinalPrice(){
    ...
    let finalPrice = this.price;
      ...
      if (!isNaN(this.price)) {
        if (this.price <= CONFIG.PRICE_MARKUP_THRESHOLD) {
          finalPrice = this.price + CONFIG.PRICE_MARKUP_LOW;
        } else {
          finalPrice = this.price + CONFIG.PRICE_MARKUP;
        }
...
class SheetManager {
  constructor(sheetName) {
    // 1. Получи активную таблицу
    const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
    
    // 2. Получи лист по имени и сохрани его в this.sheet
    this.sheet = spreadsheet.getSheetByName(sheetName);

    // 3. Проверь, что лист нашелся! Если нет - выброси ошибку.
    if (!this.sheet) {
      throw new Error(`Критическая ошибка: Лист "${sheetName}" не найден!`);
    }
    console.log(`Лист "${sheetName}" успешно найден и инициализирован.`);

    // 4. Получи все данные с листа
    const allData = this.sheet.getDataRange().getValues();
    
    // 5. Отдели заголовки и сохрани их в this.headers, а остальное - в this.data
    this.headers = allData.shift() || [];
    this.data = allData;
  }

  getHeaderMap() {
    // Сюда скопируй код из getColumnIndexMap
    // Только вместо параметра 'headers' используй 'this.headers'
    const map = {};
    this.headers.forEach((header, index) => { map[header] = index; });
    return map;
  }
}
const telegram = new TelegramService(CONFIG.TG_BOT_TOKEN, CONFIG.TG_CHAT_ID);
/**
 * =================================================================
...
function processProductPosts() {
  console.log("--- Начало обработки постов с товарами ---");

  const productSheet = new SheetManager(CONFIG.SHEET_NAMES.PRODUCTS);
  const directorySheet = new SheetManager(CONFIG.SHEET_NAMES.DIRECTORY);
  ...
  const missingColumns = requiredColumns.filter(colName => !productSheet.headers.includes(colName));
  ...
  const productColMap = productSheet.getHeaderMap();
  const directoryColMap = directorySheet.getHeaderMap();

  // Фильтрация некорректных строк
  const colGroupIndex = productColMap[CONFIG.COLUMN_NAMES.GROUP];
  const productData = productSheet.data.filter(row => row[colGroupIndex]);

  
  if (productSheet.data.length !== productData.length) {
    console.warn(`Отфильтровано ${productSheet.data.length - productData.length} строк без Названия группы или Категории.`);
  }
  ...
  directorySheet.data.forEach((row, index) => {
  ...
  if (hasStatusChanges && directorySheetManager.data.length > 0) {
    // Используем .sheet для доступа к "сырому" листу
    directorySheetManager.sheet.getRange(2, 1, directorySheetManager.data.length, directorySheetManager.headers.length).setValues(directorySheetManager.data);
  ...
  directoryMap.forEach((data, groupName) => {
    ...
    if (data.status === "Активен") {
      ...
      telegram.editMessage(messageId, messageText);
    } else {
  ...
  activeProductGroups.forEach(groupName => {
    if (!directoryMap.has(groupName)) {
      ...
      const newPostResponse = telegram.sendMessage(messageText);
      if (newPostResponse && newPostResponse.ok) {
        ...
        appendRowWithRetry(directorySheetManager.sheet, [groupName, newMessageId, "Активен"]);
      }
    }
  });
...
function processNavigationPost() {
    console.log("--- Начало обработки навигационного поста (безопасный режим) ---");
   
    const settingsSheetManager = new SheetManager(CONFIG.SHEET_NAMES.SETTINGS); // Переименуем для ясности
    const directorySheetManager = new SheetManager(CONFIG.SHEET_NAMES.DIRECTORY);
    
    
    const directoryColMap = directorySheetManager.getHeaderMap();
    
    const activePosts = directorySheetManager.data
    ...
    if (activePosts.length === 0) {
        ...
        const oldNavMessageId = getSetting(settingsSheetManager.sheet, "nav_message_id");
        if (oldNavMessageId) {
            console.log(`Активных постов нет. Удаление старого навигационного поста с ID: ${oldNavMessageId}...`);
            telegram.unpinMessage(oldNavMessageId);
            telegram.deleteMessage(oldNavMessageId);
            setSetting(settingsSheetManager.sheet, "nav_message_id", null);
        }
        return;
    }

    const { text, reply_markup } = formatNavigationPost(activePosts);
    const newPostResponse = telegram.sendMessage(text, reply_markup);
    ...
    const pinResponse = telegram.pinMessage(newNavMessageId);
    if (!pinResponse || !pinResponse.ok) {
        console.error(`Не удалось закрепить пост ${newNavMessageId}. Старый пост не будет удален.`);
        setSetting(settingsSheetManager.sheet, "nav_message_id", newNavMessageId);
        return;
    }
...
function formatProductPost(rows, colMap, groupName) {
  ...
  for (const category in groupedByCategory) {
    ...
    const product = new Product(row, colMap); // 1. Создаем умный объект

    // 2. Получаем финальную цену. Вся сложная логика спрятана в методе!
    const finalPrice = product.getFinalPrice();

    // 3. Форматируем цену для вывода
    const formattedPrice = (finalPrice === null) ? 'по запросу' : `${finalPrice.toLocaleString('ru-RU')}₽`;
      
    // 4. Собираем строку, используя чистые и понятные свойства объекта
    messageText += `${product.flag} ${product.name} – ${formattedPrice}\n`;
    });
  }
